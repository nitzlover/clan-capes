package dev.crestoria;

import dev.crestoria.api.PanelClient;
import dev.crestoria.command.ClanCapeCommand;
import dev.crestoria.command.ClanChatCommand;
import dev.crestoria.command.ClanCommand;
import dev.crestoria.events.EventScheduler;
import dev.crestoria.listener.BoundaryListener;
import dev.crestoria.listener.ClanArmorListener;
import dev.crestoria.listener.ClanCureListener;
import dev.crestoria.listener.ClanMenuListener;
import dev.crestoria.listener.ClanShieldListener;
import dev.crestoria.listener.FriendlyFireListener;
import dev.crestoria.listener.PlayerDeathListener;
import dev.crestoria.listener.PlayerJoinListener;
import dev.crestoria.placeholder.CrestoriaExpansion;
import dev.crestoria.repo.AnnouncementRepository;
import dev.crestoria.repo.ArmorTrimRepository;
import dev.crestoria.repo.BannerRepository;
import dev.crestoria.repo.ClanRepository;
import dev.crestoria.repo.EventConfigRepository;
import dev.crestoria.repo.SettingsRepository;
import dev.crestoria.task.HeartbeatTask;
import dev.crestoria.task.RefreshTask;
...
public final class CrestoriaPlugin extends JavaPlugin {
    ...
    private final List<CrestoriaExpansion> expansions = new ArrayList<>();
    ...
    public void onEnable() {
        ...
        if (Bukkit.getPluginManager().getPlugin("PlaceholderAPI") != null) {
            // Register under both ids: "crestoria" (current) + "clancapes"
            // (pre-rebrand alias) so existing %clancapes_*% placeholders in
            // TAB / LuckPerms configs survive the rename.
            for (String id : new String[] {"crestoria", "clancapes"}) {
                CrestoriaExpansion exp = new CrestoriaExpansion(this, id);
                exp.register();
                expansions.add(exp);
            }
            getLogger().info("PlaceholderAPI expansions registered (crestoria + clancapes alias).");
        } else {
    ...
    public void onDisable() {
        ...
        cancelScheduled();
        for (CrestoriaExpansion exp : expansions) {
            try {
                exp.unregister();
    ...
    /**
     * Pre-rebrand config migration. The project was "ClanCapes"; after the
     * rename the data folder is {@code plugins/Crestoria/}. If a server
     * upgraded in-place still has its panel link (url + api-key) in the old
     * {@code plugins/ClanCapes/} folder, copy those files across once so the
     * plugin stays linked without the operator re-running {@code /clancape link}.
     *
     * <p>One-shot + non-destructive: only copies when the new folder has no
     * {@code config.yml} yet AND the old one exists. Never deletes the old
     * folder — the operator removes it manually once the rename is verified.
     */
    private void migrateLegacyDataFolder() {
        try {
            java.nio.file.Path newDir = getDataFolder().toPath();
            if (java.nio.file.Files.exists(newDir.resolve("config.yml"))) return; // already set up
            java.nio.file.Path legacyDir = newDir.getParent().resolve("ClanCapes");
            if (!java.nio.file.Files.exists(legacyDir.resolve("config.yml"))) return; // fresh install
            java.nio.file.Files.createDirectories(newDir);
            try (var stream = java.nio.file.Files.walk(legacyDir)) {
                stream.forEach(src -> {
                    try {
                        java.nio.file.Path dst = newDir.resolve(legacyDir.relativize(src));
                        ...
                        getLogger().warning("[migrate] skip " + src.getFileName() + ": " + perFile.getMessage());
                    }
                });
            }
            getLogger().warning("Migrated legacy plugins/ClanCapes/ into plugins/Crestoria/ "
                    + "(pre-rebrand upgrade). Old folder left intact — delete it manually once verified.");
        } catch (Throwable t) {
            getLogger().warning("[migrate] legacy data-folder migration failed: " + t.getMessage());
        }
    }
    ...
    private void tryAutoDownload(String latest, String downloadUrl) {
        ...
        java.nio.file.Path target = updateDir.resolve("Crestoria-" + latest + ".jar");
        getServer().getScheduler().runTaskAsynchronously(this, () -> {
            try {
                ...
                java.net.http.HttpRequest req = java.net.http.HttpRequest.newBuilder()
                        ...
                        .header("User-Agent", "Crestoria/" + getPluginMeta().getVersion())
                        .GET()
                        .build();
