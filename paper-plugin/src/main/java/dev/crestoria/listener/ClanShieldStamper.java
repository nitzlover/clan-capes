...
public final class ClanShieldStamper {
    ...
    public static final NamespacedKey SHIELD_OWNER_KEY =
            new NamespacedKey("crestoria", "shield_owner");
    /**
     * Pre-rebrand key (project was "ClanCapes"). Read-only fallback so shields
     * stamped before the Crestoria rename stay recognised; re-stamping migrates
     * them onto {@link #SHIELD_OWNER_KEY}.
     */
    public static final NamespacedKey LEGACY_SHIELD_OWNER_KEY =
    ...
    public static String readMarker(BlockStateMeta meta) {
        ...
        if (pdc.has(SHIELD_OWNER_KEY, PersistentDataType.STRING)) {
            return pdc.get(SHIELD_OWNER_KEY, PersistentDataType.STRING);
        }
        if (pdc.has(LEGACY_SHIELD_OWNER_KEY, PersistentDataType.STRING)) {
            return pdc.get(LEGACY_SHIELD_OWNER_KEY, PersistentDataType.STRING);
        }
        return null;
    }
    ...
    public static boolean apply(ItemStack shield, BannerDto banner, String clanTag,
        ...
        meta.getPersistentDataContainer().remove(LEGACY_SHIELD_OWNER_KEY); // migrate off pre-rebrand key
    ...
    public static boolean stripIfOurs(ItemStack shield) {
        ...
        meta.getPersistentDataContainer().remove(LEGACY_SHIELD_OWNER_KEY);
        shield.setItemMeta(meta);
        return true;
    }
     ...
     */
    private static List<Pattern> parsePatterns(BannerDto banner, CrestoriaPlugin plugin) {
