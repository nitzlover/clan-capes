...
public final class StatsCache {
    ...
    private void scheduleRefresh(UUID uuid) {
        // De-duplicate concurrent refresh attempts via a marker entry.
        Entry placeholder = entries.computeIfAbsent(uuid, k -> new Entry(0, 0, 0, 0, true, 0L));
        if (placeholder.refreshing) {
            // A fresh entry just got minted — no work for us.
        }
