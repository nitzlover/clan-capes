...
// Seated + both arms held out front (captured 1:1 from the v9 login's roast pose).
// Pair with the "marshmallow stick" toggle to see the full gesture.
const MC_ROAST: NodeXform = {
  ...MC_SEATED,
  ArmRightUpper: { ...MC_SEATED.ArmRightUpper, rotation: [2.59329, -0.04557, -0.34555] },
  ArmRightLower: { ...MC_SEATED.ArmRightLower, rotation: [0.5, 0, 0] },
...
// Shared studio shell. `/studio` renders it with mode='create' (pose authoring:
// the poses panel / new pose / AI studio / library); `/p/[id]` with mode='render'
// (render customisation: scene / camera / other + download). Same 3D engine,
// canvas, and bottom-left toolbar in both. Plain module (NOT a route page) so it
// can be imported by both route files.
export function StudioShell({ mode }: { mode: 'create' | 'render' }) {
  ...
  const [stickOn, setStickOn] = useState(false); // roasting stick + marshmallow
  ...
  useEffect(() => { apiRef.current?.setRoastStick(stickOn); }, [stickOn]);
  ...
  // Cape and elytra share the Minecraft cape slot — mutually exclusive.
  // Picking a cape clears the elytra; enabling the elytra clears the cape.
  const pickCape = (url: string | null) => { setCape(url); if (url) setElytra(false); };
  const toggleElytra = (v: boolean) => { setElytra(v); if (v) setCape(null); };
  ...
  const acceptSkin = useCallback(async (nameArg?: string) => {
    const v = (typeof nameArg === 'string' ? nameArg : username).trim();
  ...
  // ── two-page flow: /studio (author the pose) → /p (render settings) ──
  // Carry the whole character (pose + skin + accessories + nametag + render
  // opts) to the render page via sessionStorage; restore it there once booted.
  const STUDIO_SNAP_KEY = 'studio:pose-snapshot';
  const goRender = useCallback(() => {
    try {
      sessionStorage.setItem(STUDIO_SNAP_KEY, JSON.stringify({
        pose, modelKind, username, cape, capeCustom, hat, armor, elytra,
        mainItem, offItem, heldXf, nametag, secondLayer, layers3d, fx,
      }));
    } catch { /* no sessionStorage — render page just boots default */ }
    router.push('/p/local');
  }, [pose, modelKind, username, cape, capeCustom, hat, armor, elytra, mainItem, offItem, heldXf, nametag, secondLayer, layers3d, fx, router]);

  const restoredRef = useRef(false);
  useEffect(() => {
    if (mode !== 'render' || status !== 'ready' || restoredRef.current) return;
    restoredRef.current = true;
    let snap: ReturnType<typeof JSON.parse> = null;
    try { snap = JSON.parse(sessionStorage.getItem(STUDIO_SNAP_KEY) || 'null'); } catch { snap = null; }
    if (!snap) return;
    if (snap.pose) setPose(snap.pose);
    if (snap.modelKind) setModelKind(snap.modelKind);
    if (snap.cape !== undefined) setCape(snap.cape);
    if (snap.capeCustom !== undefined) setCapeCustom(snap.capeCustom);
    if (snap.hat !== undefined) setHat(snap.hat);
    if (snap.armor) setArmor(snap.armor);
    if (typeof snap.elytra === 'boolean') setElytra(snap.elytra);
    if (snap.mainItem !== undefined) setMainItem(snap.mainItem);
    if (snap.offItem !== undefined) setOffItem(snap.offItem);
    if (snap.heldXf) setHeldXf(snap.heldXf);
    if (snap.nametag) setNametag(snap.nametag);
    if (typeof snap.secondLayer === 'boolean') setSecondLayer(snap.secondLayer);
    if (snap.layers3d) setLayers3d(snap.layers3d);
    if (snap.fx) setFx(snap.fx);
    if (snap.username) { setUsername(snap.username); setSkinApplied(true); void acceptSkin(snap.username); }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mode, status, acceptSkin]);
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <section className="relative min-h-0 flex-1 overflow-hidden">
              ...
              <div className="relative h-full min-h-0 w-full overflow-hidden">
                ...
                {/* two-page flow CTA: author → render (create) / back to studio (render) */}
                {mode === 'create' ? (
                  <button onClick={goRender}
                    className="absolute left-3 top-3 z-30 flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-black shadow-md transition hover:bg-white/90">
                    render <ArrowRight className="h-3.5 w-3.5" />
                  </button>
                ) : (
                  <button onClick={() => router.push('/studio')}
                    className="absolute left-3 top-3 z-30 flex items-center gap-1.5 rounded-lg border border-zinc-700 bg-zinc-900/90 px-3 py-1.5 text-xs font-medium text-zinc-200 shadow-md backdrop-blur-sm transition hover:bg-zinc-800">
                    <ArrowRight className="h-3.5 w-3.5 rotate-180" /> studio
                  </button>
                )}
                ...
                <div className="absolute bottom-3 left-3 z-30">
                  {openTool && (
                    <div className="absolute bottom-full left-0 mb-2 rounded-lg border border-zinc-700/80 bg-zinc-900/95 p-3.5 shadow-2xl backdrop-blur-md">
                      ...
                      {openTool === 'armor' && (
                        <div className="w-64">
                          <ArmorSlots armor={armor} setArmor={setArmor} elytra={elytra} setElytra={toggleElytra} />
                        </div>
                      )}
                    </div>
                  )}
        ...
        <aside className="flex min-h-0 w-full flex-none flex-col border-t border-zinc-800 sm:w-[340px] sm:border-l sm:border-t-0 lg:w-[360px]">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.keys(NODE_POSES).map((k) => (
                      <CamBtn key={k} onClick={() => { apiRef.current?.setNode(NODE_POSES[k]); if (k === 'Roast') setStickOn(true); }}>{k} ★</CamBtn>
                    ))}
                    {Object.keys(PRESETS).map((k) => (
                      <CamBtn key={k} onClick={() => setPose(PRESETS[k])}>{k}</CamBtn>
                    ))}
                    <CamBtn onClick={() => { setPose(ZERO_POSE()); setStickOn(false); }}>reset</CamBtn>
                    <CamBtn onClick={() => setStickOn((v) => !v)}>{stickOn ? 'stick ✓' : '+ stick'}</CamBtn>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    {capeCustom && (
                      <button type="button" onClick={() => pickCape(capeCustom)} aria-label="select custom cape" title="custom"
                    ...
                    {(capesExpanded ? CAPES : CAPES.slice(0, 5)).map((id) => {
                      ...
                      return (
                        <button key={id} type="button" onClick={() => pickCape(url)} title={id} aria-label={`select ${id} cape`}
                  ...
                  <input ref={capeFileRef} type="file" accept="image/png,image/jpeg,image/webp" className="hidden" aria-hidden="true" tabIndex={-1}
                    onChange={(e) => {
                      ...
                      r.onload = () => { const url = String(r.result); setCapeCustom(url); pickCape(url); };
                      r.readAsDataURL(f);
                      e.target.value = '';
                    }} />
                </div>
          ...
          {/* download bar — render mode only (the pose page) */}
          {mode === 'render' && (
...
// Sliding pill toggle — radix <Switch> look, B&W Clan Capes palette.
function Switch({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)}
      className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border transition-colors focus:outline-none focus:ring-2 focus:ring-zinc-500/40 ${checked ? 'border-white bg-white' : 'border-zinc-600 bg-zinc-800'}`}>
      <span className={`pointer-events-none block size-3.5 rounded-full shadow transition-transform ${checked ? 'translate-x-[18px] bg-zinc-900' : 'translate-x-[3px] bg-zinc-400'}`} />
    </button>
  );
}

// One option row inside a SlotSelect dropdown (onMouseDown so the trigger's
// onBlur doesn't swallow the pick before it fires).
function SlotOpt({ active, onPick, children }: { active: boolean; onPick: () => void; children: string }) {
  return (
    <button type="button" onMouseDown={onPick}
      className={`flex w-full items-center justify-between rounded-[5px] px-2 py-1.5 text-left text-xs lowercase transition hover:bg-zinc-800 ${active ? 'text-white' : 'text-zinc-300'}`}>
      <span>{children}</span>
...
// A radix <Select>-style combobox for one armor slot.
function SlotSelect({ label, value, tiers, onChange }: { label: string; value: string | null; tiers: readonly string[]; onChange: (v: string | null) => void }) {
  ...
  return (
    <div className="relative">
      <div className="mb-1.5 text-[11px] lowercase text-zinc-500">{label}</div>
      <button type="button" onClick={() => setOpen((o) => !o)} onBlur={() => setTimeout(() => setOpen(false), 120)}
        className="flex h-9 w-full items-center justify-between gap-1 rounded-md border border-zinc-700 bg-zinc-950 px-2.5 text-xs lowercase text-zinc-200 transition hover:bg-zinc-800/60 focus:outline-none focus:ring-2 focus:ring-zinc-500/40">
        <span className={value ? 'text-zinc-100' : 'text-zinc-500'}>{value ?? 'none'}</span>
        <ChevronDown className="h-3.5 w-3.5 shrink-0 text-zinc-500" />
      </button>
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 max-h-52 w-full overflow-y-auto rounded-md border border-zinc-700 bg-zinc-900 p-1 shadow-2xl">
          <SlotOpt active={!value} onPick={() => pick(null)}>none</SlotOpt>
          {tiers.map((t) => (
            <SlotOpt key={t} active={value === t} onPick={() => pick(t)}>{t}</SlotOpt>
          ))}
        </div>
      )}
    </div>
...
// Armor & elytra controls — mcrender popover layout: an "armor slots" header,
// a 2-col helmet/chestplate/leggings/boots grid, then a bordered elytra row
// with a wind icon + a Switch toggle.
function ArmorSlots({ armor, setArmor, elytra, setElytra }: { armor: ArmorSlots; setArmor: (a: ArmorSlots) => void; elytra: boolean; setElytra: (v: boolean) => void }) {
  ...
  return (
    <div className="space-y-3">
      <div className="text-[11px] font-medium lowercase tracking-wide text-zinc-500">armor slots</div>
      ...
      <div className="flex items-center justify-between rounded-lg border border-zinc-700 px-3 py-2.5">
        <span className="flex items-center gap-2 text-xs lowercase text-zinc-200">
          <Wind className="h-4 w-4 text-zinc-400" /> elytra
        </span>
        <Switch checked={elytra} onChange={setElytra} />
      </div>
    </div>
  );
}
...
export async function bootViewer(mount: HTMLDivElement, isDisposed: () => boolean): Promise<StudioApi | null> {
  ...
  // Pan ON (mcrender behaviour): right-drag / two-finger drag translates the
  // subject in-frame without changing the orbit angle. (enablePan defaults true.)
  ...
  const buildRig = (scn: import('three').Object3D) => {
    ...
    scn.updateWorldMatrix(true, true);
    scn.traverse((o) => { if (o.name) bone[o.name] = o; });
    // Centre the rig on X/Z using the REAL head-bone world position. `Box3.setFromObject`
    // is garbage for SkinnedMeshes — for the slim `pfp.gltf` it shoved the rig ~6 units
    // back in Z (head landed at z≈-2.4), so the character drifted off-frame while the
    // nametag (fixed at modelGroup [0, NAMETAG_Y, 0]) stayed centred. The head must sit at
    // world X/Z = 0. Done BEFORE capturing restWorldByBone so rigid armor/elytra attach to
    // the corrected rest matrices.
    {
      const hb = bone['Head'];
      if (hb) {
        scn.updateWorldMatrix(true, true);
        const hp = new THREE.Vector3(); hb.getWorldPosition(hp);
        scn.position.x -= hp.x; scn.position.z -= hp.z;
        scn.updateWorldMatrix(true, true);
      }
    }
    scn.traverse((o) => { if (o.name) { restQ[o.name] = o.quaternion.clone(); restPos[o.name] = o.position.clone(); restWorldByBone[o.name] = o.matrixWorld.clone(); } });
  ...
  // Roasting stick + marshmallow held in the right hand (pairs with the 'Roast'
  // pose). A long box + a cube parented to the forearm, extending forward.
  let roastStickObj: import('three').Object3D | null = null;
  const setRoastStick = (on: boolean) => {
    if (roastStickObj) { roastStickObj.parent?.remove(roastStickObj); roastStickObj = null; }
    if (!on) return;
    const b = bone['ArmRightLower'];
    if (!b) return;
    const grp = new THREE.Group();
    const L = 2.2; // stick length (bone-local units)
    const HAND = 0.5; // ≈ hand position up the forearm (+Y = toward the hand)
    const stickMat = new THREE.MeshStandardMaterial({ color: 0x4a3b29, roughness: 1, metalness: 0 });
    // stick extends along the forearm's +Y axis (continues out of the hand → the
    // forward-down "roasting" angle once the Roast pose is applied).
    const stick = new THREE.Mesh(new THREE.BoxGeometry(0.05, L, 0.05), stickMat);
    stick.position.set(0, HAND + L / 2, 0);
    grp.add(stick);
    const mMat = new THREE.MeshStandardMaterial({ color: 0xf3ede0, roughness: 0.85, metalness: 0, emissive: 0x2a1e10, emissiveIntensity: 0.3 });
    const marsh = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.2, 0.2), mMat);
    marsh.position.set(0, HAND + L * 0.96, 0);
    grp.add(marsh);
    b.add(grp);
    // Aim the stick (its local +Y) at the character's FRONT + slightly down — the
    // roasting angle — regardless of how the forearm bone is oriented by the pose.
    b.updateWorldMatrix(true, false);
    const wq = new THREE.Quaternion(); b.getWorldQuaternion(wq);
    const localDir = new THREE.Vector3(0, -0.42, 1).normalize().applyQuaternion(wq.invert());
  ...
  const setArmor = async (slots: ArmorSlots) => {
    ...
    // mcrender: when the elytra is ON *and* a chestplate is equipped, the chestplate's
    // chest+arm L1 plates are suppressed — the elytra occupies that slot, so you see the
    // wings INSTEAD of the chest plate (helmet/leggings/boots are unaffected).
    const suppressChest = elytraOn && !!slots.chestplate;
    const CHEST_PIECES = new Set(['body', 'left_arm', 'right_arm']);
    ...
    for (const piece of [...L1_ORDER, ...L2_ORDER]) {
      ...
      if (!mat) continue;
      if (suppressChest && CHEST_PIECES.has(piece)) continue; // elytra hides the chestplate body/arms
  ...
  // Elytra — mcrender's EXACT `ArmorLayers` recipe (reverse-engineered 1:1 from chunk
  // 0rtzdmvo1i-go.js): each wing is a plain rigid Mesh (NOT skinned, NOT baked) whose local
  // transform is decomposed from inverse(elytraScene.matrixWorld)·wingNode.matrixWorld (yields
  // a Ry(180°) per wing); both wings go in one `elytraRoot` group with the hardcoded root
  // pos/rot/scale below, then the group is parented into the **Chest** bone (Body's +90°X and
  // MAIN's −90°X cancel there → world-axis-aligned, so these world-style Euler angles land the
  // wings folded flat on the back; attaching to Body instead inherits the uncancelled +90°X and
  // the wings jut forward — that was the bug). renderOrder 46 (between L1 42.x and L2 50.x).
  // rz IS 0.1 (mcrender's literal `Math.PI+.36, Math.PI+.04, .1`); the earlier rz=0 was wrong.
  ...
  const buildElytra = async () => {
    ...
    g.scene.traverse((o) => {
      ...
      mat.side = THREE.DoubleSide; mat.flatShading = true; mat.roughness = 1; mat.metalness = 0;
      mat.alphaTest = 0.1; mat.transparent = false; mat.envMapIntensity = 0;
  ...
  const setElytra = async (on: boolean) => {
    elytraOn = on;
    await buildElytra();
    await setArmor(curSlots); // re-gate: hide the chestplate's chest+arms (on) or restore them (off)
  };
  ...
  return {
    setPose, setNode, setRoastStick, setSkin, setCamera, setModel,
